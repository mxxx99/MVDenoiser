...
from check_gpu_my import wait_for_allow_execution
use_gpu=wait_for_allow_execution(gpus,0.95)
...
save_checkpoint,denoise_shuffle,raw2rgb_crvd,raw2rgb_rnvd
...
def main():
    ...
    #pretrain:[35,60,80,100,120]#tune_crvd:[35,55,70,85]#tune_v2_new=[120,140,160,180]#rnvd_tune=[80,100,120]
    parser.add_argument('--co_training', default=False, help='if train with optical flow')
    parser.add_argument('--use_syn', default=True, help='if use synthetic noise for training')
    parser.add_argument('--res_learn', default=False, help='use noise or noisy for supervision')
    parser.add_argument('--save_for_srga',default=False)
    parser.add_argument('--stage',default='tune',help='pretrain_clean,pretrain_noisy,\
                        mask_noisy2clean or tune')
    parser.add_argument('--input_mask',default='maskall_singlechannel')
    parser.add_argument('--mask_ratio',default=[80,100])
...
def run_test(gpu, args, test_loader, model, criterion, keys, save=False):
    print('running test')
    if save==True:
        savepath='/data3/mxx/Noise_generate/Starlight_ours_older/results/%s_%s/'%(args.network,args.dataset)
    ...
    with torch.no_grad():
        ...
        for i, sample in enumerate(test_loader): #Loop through dataset
            ...
            if args.stage=='pretrain_clean':#用干净视频pretrain
                net_input=gt_label.clone()
            else:
                net_input = noisy_raw.clone()

            noise_levels={}
            if not len(keys)==0:
                for key in sample['noisy_inter']:
                    noise_levels[key]=sample['noise_levels'][key].cuda(non_blocking=True).view(B,1,1,1,1)
            pos=sample['pos']   # 裁剪位置
            # noise_level=sample['noise_level'][0].item()
            if save==True:
                frame_name=sample['frame_name'][0].split('.')[0]
            ...
            if args.save_for_srga:
                if not os.path.exists('%s_clean'%args.dataset):
                    os.mkdir('%s_clean'%args.dataset)
                torch.save(feat_inter, '%s_clean/%d.pt'%(i,args.dataset))
            
            # test loss暂时只算最终的
            if args.stage=='pretrain_clean':
                test_loss = criterion(torch.mul(denoise_all,mask), torch.mul(gt_label[:,:,2:-2],mask))
            elif args.stage=='pretrain_noisy':
                test_loss = criterion(torch.mul(denoise_all,mask), torch.mul(noisy_raw[:,:,2:-2],mask))
            elif args.stage=='mask_noisy2clean':
            ...
            if save==True:
                if args.dataset=='CRVD':
                    out_plt = raw2rgb_crvd(denoise_all[:,:,0])#(b,c,t,h,w)->(h,w,c)
                    # torch.save(out_plt,'%s%s_noise%s_psnr%.4f_ssim%.4f.pt'%
                    #                                         (savepath,frame_name,noise_level,cur_test_psnr,cur_test_ssim))
                elif args.dataset=='RNVD':
                    out_plt = raw2rgb_rnvd(denoise_all[:,:,0])#(b,c,t,h,w)->(h,w,c)
                Image.fromarray(out_plt#只能对一个scene存，我没写对不同scene的情况
                                .astype(np.uint8)).save('%s%s_psnr%.4f_ssim%.4f.png'%
                                                        (savepath,frame_name,cur_test_psnr,cur_test_ssim))
            else:
...
def write_logs(string,log_dir='log_80_100.txt'):
...
def train(gpu, args):
    ...
    # scheduler_G = WarmupMultiStepLR(optimizer_G, warmup_steps=10,milestones=args.milestone,gamma=0.5,last_epoch=curr_epoch-1)
    ...
    for epoch in range(curr_epoch, args.num_iter):
        ...
        for i, sample in enumerate(train_loader):
            ...
            noisy_raw = sample['noisy_input'].cuda(non_blocking=True)
            if args.stage=='pretrain_clean':
                net_input=clean_raw.clone().detach()
            else:#pretrain_noisy或者是mask_noisy2clean或者是tune都是noisy输入
                net_input = noisy_raw.clone().detach()
            noise_inter={};noise_levels={}
            if not len(keys)==0: 
                for key in sample['noisy_inter']:
                    noise_inter[key]=gh.t32_1(sample['noisy_inter'][key][:,:,2:-2].cuda(non_blocking=True))
                    noise_levels[key]=sample['noise_levels'][key].cuda(non_blocking=True).view(B,1,1,1,1)
            ...
            if len(keys)>0:
                if epoch<discriminator_epoch:
                    ...
                    with torch.no_grad():
                        # 训练判别器时不需要生成器的梯度
                        gen_denoise_all,gen_noise_inter,gen_predicted_noise,_,_ = model(net_input,clean_raw,noise_level=noise_levels)
            ...
            if i % n_critic == 0:
                ...
                fake_denoise_all,denoised_inter,predicted_noise,_,mask = model(net_input,clean_raw,noise_level=noise_levels)
                if not mask==None:
                    mask=1.0-mask.unsqueeze(2)
                ...
                if args.stage=='pretrain_clean':
                    ...
                    g_loss+=lambda3*denoiser_loss(torch.mul(fake_denoise_all,mask),torch.mul(noisy_raw[:,:,2:-2],mask))
                elif args.stage=='mask_noisy2clean':#和masked那篇策略一致
        ...
        if gpu == 0:
            avg_test_loss, test_psnr, out_plt = run_test(gpu, args, test_loader, model, denoiser_loss, keys)
...
def eval(gpu, args):
    ...
    print('Testing')
    avg_test_loss, test_psnr, out_plt = run_test(gpu, args, test_loader, model, criterion, keys)
