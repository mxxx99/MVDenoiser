...
def extract_flow_torch(model, im1_torch, im2_torch, downscale,co_training=True):
    '''return:(b,2,h,w)'''
    ...
    new_H = int(math.floor(math.ceil((H/downscale) / 64.0) * 64.0))
    new_W = int(math.floor(math.ceil((W/downscale) / 64.0) * 64.0))
